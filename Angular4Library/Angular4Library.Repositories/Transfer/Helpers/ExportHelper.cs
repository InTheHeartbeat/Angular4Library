    ...
    {        
        public static byte[] ExportBooks(ExportModel model, RepositoryProvider repositoryProvider)
        {
            ...
            if (model.IsXml)
            {
                ...
                serializer.Serialize(stream, model.IdsArray.Select(id => repositoryProvider.BooksRepository.GetProductById(id)).Select(book => new ExtendedBook()
                {
                    Amount = book.Amount,
                    Genre = book.Genre,
                    Pages = book.Pages,
            ...
            {                
                ...
                foreach (ExtendedBook book in model.IdsArray.Select(id => repositoryProvider.BooksRepository.GetProductById(id)))
                {                   
